...
//        System.out.println("gcd = " + gcd(cipherA.longValue(),nsqr.longValue()));
    ...
    public BigInteger[] homo_add(BigInteger[] cipher1, BigInteger[] cipher2){
    	...
    	new_cipher[0]=cipher1A.multiply(cipher2A);
    	new_cipher[1]=cipher1B.multiply(cipher2B);
    	return new_cipher;
    }
     ...
     * @throws CryptoException */
    public BigInteger[] secu_multi(BigInteger[] cipherA, BigInteger[] cipherB) throws CryptoException{
    	BigInteger plainA =this.decrypt(cipherA);
    	BigInteger plainB =this.decrypt(cipherB);
    	System.out.println("Secu multi received@Proxy "+plainA+" "+plainB);
    	BigInteger[] result = this.encrypt(plainA.multiply(plainB));
    	System.out.println("Secu multi calculated@Proxy "+result[0]+" "+result[1]);
    	return result;
    }
     ...
     * secure division
     * @throws CryptoException */
    public BigInteger[] secu_div(BigInteger[] Numerator, BigInteger[] Denominator) throws CryptoException{
    	BigInteger plainA =  this.decrypt(Numerator);
    	BigInteger plainB = this.decrypt(Denominator);
    	System.out.println("Secu div paras received@Proxy "+plainA+" "+plainB);
    	//covert BigInteger to double to compute division
    	double temp = plainA.doubleValue()/((this.bigsqrt(plainB)).doubleValue());
    	BigInteger rp =BigInteger.valueOf(Math.round(temp*10));
    	BigInteger[] rcipher = this.encrypt(rp);
    	System.out.println("Secu div calculated@Proxy "+rcipher[0]+" "+rcipher[1]);
    	return rcipher;
    }
    
    /**
     * @author kasaquan
     * compute the sqrt of BigInteger
     * this is the Floor Algorithm, there is also a ceil version*/
    public BigInteger bigsqrt(BigInteger x)throws IllegalArgumentException{
